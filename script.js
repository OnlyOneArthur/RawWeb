...
// ===== Products demo =====
const PRODUCTS = [
  { id: 'av-1', name: 'AllSafe Antivirus Pro', cat:'antivirus', price:39, image:'https://via.placeholder.com/640x480?text=Antivirus' },
  { id: 'vpn-1', name: 'AllSafe VPN',          cat:'vpn',       price:49, image:'https://via.placeholder.com/640x480?text=VPN' },
  { id: 'fw-1',  name: 'AllSafe Firewall',     cat:'firewall',  price:59, image:'https://via.placeholder.com/640x480?text=Firewall' },
  { id: 'cl-1',  name: 'AllSafe Cloud Guard',  cat:'cloud',     price:79, image:'https://via.placeholder.com/640x480?text=Cloud+Security' },
];
...
function applyFilters(){
  ...
  switch(sort){
    ...
    case 'popularity': list.sort((a,b)=>a.name.localeCompare(b.name)); break;
...
function addPurchase(p, qty){
  ...
  history.unshift({product:p.name, date:new Date(), qty, price:p.price, total});
  renderHistory();
}
...
function renderHistory(){
  ...
  switch(sort){
    case 'date-new': rows.sort((a,b)=>b.date-a.date); break;
    case 'date-old': rows.sort((a,b)=>a.date-b.date); break;
    case 'total-high': rows.sort((a,b)=>b.total-a.total); break;
    case 'total-low': rows.sort((a,b)=>a.total-b.total); break;
  }
...
(()=>{
  ...
  const setIntent = (intent) => {
    ...
    authLink.setAttribute('aria-label', intent === 'login' ? 'Login' : 'Register (hover to Login)');
  };
